...
public class ProductosServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pathInfo == null || "/".equals(pathInfo)) {
			...
			//	/productos/
			//	/productos
			String palabra = request.getParameter("filtrar-por-nombre");

			if (palabra != null) {
				List<Producto> listadoFiltrado = proDAO.geProductosFiltrados(palabra);
				request.setAttribute("listaProductos", listadoFiltrado);
			...
			if (pathParts.length == 2 && "crear".equals(pathParts[1])) {
				...
				// GET
				// /productos/crear
				request.setAttribute("listFabricantesNombres", proDAO.getNombresFabsAll());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (__method__ == null) {
			// Crear uno nuevo
			ProductoDAO prDAO = new ProductoDAOImpl();
			...
			nuevoP.setPrecio(Double.parseDouble(precio));
			nuevoP.setCodigo_fabricante(Integer.parseInt(idF));
