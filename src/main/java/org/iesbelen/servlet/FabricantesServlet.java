...
public class FabricantesServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pathInfo == null || "/".equals(pathInfo)) {
			...
			List<FabricanteDTO> listaFabricantes;

			listaFabricantes = fabDAO.getAll().stream()
					.map(FabricanteDTO::new)
					.toList();
			listaFabricantes.forEach(FabricanteDTO::setCantProductos);

			if (ordenarPor != null && modoOrdenar != null) {
				if (ordenarPor.equalsIgnoreCase("nombre") && modoOrdenar.equalsIgnoreCase("asc")){
					listaFabricantes = listaFabricantes.stream()
							.sorted((o1, o2) -> o1.getNombre().compareTo(o2.getNombre()))
							.toList();
				} else if (ordenarPor.equalsIgnoreCase("nombre") && modoOrdenar.equalsIgnoreCase("desc")){
					listaFabricantes = listaFabricantes.stream()
							.sorted((o1, o2) -> o2.getNombre().compareTo(o1.getNombre()))
							.toList();
				}
				if (ordenarPor.equalsIgnoreCase("codigo") && modoOrdenar.equalsIgnoreCase("asc")){
					listaFabricantes = listaFabricantes.stream()
							.sorted((o1, o2) -> o1.getIdFabricante() - o2.getIdFabricante())
							.toList();
				} else if (ordenarPor.equalsIgnoreCase("codigo") && modoOrdenar.equalsIgnoreCase("desc")){
					listaFabricantes = listaFabricantes.stream()
							.sorted((o1, o2) -> o2.getIdFabricante() - o1.getIdFabricante())
							.toList();
				}
			}
			...
			if (pathParts.length == 2 && "crear".equals(pathParts[1])) {
				...
				// /fabricantes/{id}
				try {
					Optional<Fabricante> fabricante = fabDAO.find(Integer.parseInt(pathParts[1]));
					FabricanteDTO fabDTO = null;

					if (fabricante.isPresent()) {
						fabDTO = new FabricanteDTO(fabricante.get());
						fabDTO.setCantProductos();
					}
