...
public class ProductoDAOImpl extends AbstractDAOImpl implements ProductoDAO{
	...
	public List<Fabricante> getNombresFabsAll() {
		...
		try {
			conn = connectDB();
			ps = conn.prepareStatement("SELECT * FROM fabricantes");
			rs = ps.executeQuery();

			while (rs.next()) {
				int idx = 1;
				Fabricante fabricante = new Fabricante();
				fabricante.setIdFabricante(rs.getInt(idx++));
				fabricante.setNombre(rs.getString(idx++));
				result.add(fabricante);
			}
	...
	public List<Producto> geProductosFiltrados(String nombre) {
		...
		try {
			...
			while (res.next()) {
				index = 1;
				Producto prod = new Producto();
				prod.setIdProducto(res.getInt(index++));
				prod.setNombre(res.getString(index++));
				prod.setPrecio(res.getDouble(index++));
				prod.setCodigo_fabricante(res.getInt(index++));
